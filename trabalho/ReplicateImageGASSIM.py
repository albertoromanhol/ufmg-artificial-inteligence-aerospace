...
def calculate_fitness(target_image, solution):
    image =  chromossome_to_image(solution, target_image.shape)
    ssimResult = ssim(target_image, image, multichannel=True)

    return ssimResult

def calculate_population_fitness(target_image, population):
    qualities = np.zeros(population.shape[0])

    for i in range(population.shape[0]):
        qualities[i] = calculate_fitness(target_image, population[i, :])
...
def run(image, generation_size, population_size, mutation_percent, gaussian_filter = False):
    ...
    for iteration in range(generation_size + 1):
        qualities = calculate_population_fitness(target_image, population)
...
# %%
run('apple_low_res.jpg', 20000, 128, 0.05, True)